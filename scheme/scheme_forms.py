...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        py_func = LambdaProcedure(signature.rest, expressions.rest, env)
        env.define(signature.first, py_func)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return "#t"
    for expr in pair_iter(expressions):
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    let_env = Frame(env)
    for binding in pair_iter(bindings):
        validate_form(binding, 2, 2)
        if not let_env.bindings.get(binding.first, None) is None:
            raise SchemeError
        if not type(binding.first) is str:
            raise SchemeError
        let_env.define(binding.first, scheme_eval(binding.rest.first, env))
    return let_env
    # END PROBLEM 14
