...
class Frame:
    ...
    def mass_define (self, symbols, values):
        """
        Define each symbol in symbols to each value in values respectively 
        (Student self-define method)
        Parameters
        ----------
        symbols : Scheme list (Pair)
        values : Scheme list (Pair)
        """
        vals_iter = pair_iter(values)
        symbols_iter = pair_iter(symbols)
        try:
            self.bindings.update({next(symbols_iter): val for val in vals_iter})
    ...
    def make_child_frame(self, formals, vals):
        ...
        "*** YOUR CODE HERE ***"
        child = Frame(self)
        child.mass_define(formals, vals)
        return child
        # END PROBLEM 8
