...
def boar_brawl(player_score, opponent_score):
    ...
    def tens_digit(x):
        x = x - (x % 10)
        return int((x%100)/10)
    return max(abs(tens_digit(opponent_score) - ones_digit(player_score))*3, 1)
    # END PROBLEM 2
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # END PROBLEM 4
print(  sus_update(2, 2, 3, make_test_dice(4)))
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    former_num_rolls = strategy(0,0)
    for i in range(goal):
        for k in range(goal):
            if strategy(i,k) != former_num_rolls:
                return False
    return True
    # END PROBLEM 7
