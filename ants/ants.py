...
class ShiftedPlaceOutOfBoundError(Exception):
    pass
...
#Debug Place
place0 = Place('0')
place1 = Place('1', place0)
place2 = Place('2', place1)
place3 = Place('3', place2)
place4 = Place('4', place3)
place5 = Place('5', place4)
place6 = Place('6', place5)
...
class Ant(Insect):
    ...
    def __init__(self, health=1):
        ...
        self.has_queen_inspiration = False
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if not self.has_queen_inspiration:
            self.damage *= 2
            self.has_queen_inspiration = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        try:
            ...
            if self.lower_bound == None:
                ...
                limit = self.upper_bound - self.lower_bound
            return random_bee(linear_sight(
                    get_shifted_place(self.place, self.lower_bound),
                    limit))
        except ShiftedPlaceOutOfBoundError:
            return None
    # def nearest_bee(self):
    #     """Return the nearest Bee in a Place (that is not the hive) connected to
    #     the ThrowerAnt's Place by following entrances.
    ...
    #     return random_bee(linear_sight(self.place))
    #     # END Problem 3 and 4
...
def linear_sight(place, limit = None):
    """
    NOTE: This function is not included in skeleton code
    Parameters
    place : Place object
    -------
    Returns
    List: list of Bee Objects at the closest Place object to place, while not 
    exceeding the limit
    """
    if len(place.get_bees()) != 0:
        return place.get_bees()
    elif place.entrance == None or place.entrance.is_hive == True:
        return []
    elif limit > 0:
        return linear_sight(place.entrance, limit - 1)
    elif limit == 0:
        return []

def get_shifted_place(place, n):
    """
    Parameters
    ----------
    place : Place object
    ...
    Returns
    -------
    A Place object that is n places far from place 
    (if n>0 than the return place object will be on the entrance side of place)
    """
    if n == 0 or n == None:
        return place
    if n > 0:
        if place.entrance != None:
            return get_shifted_place(place.entrance, n-1)
        raise ShiftedPlaceOutOfBoundError()
    elif n < 0:
        if place.exit != None:
            return get_shifted_place(place.exit, n-1)
        raise ShiftedPlaceOutOfBoundError()
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # if self.health <= 0:
        #     total_damage += self.damage
        # for bee in self.place.get_bees():
        #     bee.reduce_health(total_damage)
        # if self.health <= 0:
        #     self.zero_health_callback()
        #     self.place.remove_insect(self)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.turns_to_chew <= 0 and len(self.place.get_bees()) != 0:
            unlucky_bee = random_bee(self.place.get_bees())
...
# END Problem 11
def find_behind_ants(place):
    """
    Parameters
    place : Place object
    -------
    Returns
    List: list of Ant objects at the exit direction of place
    """
    ants = []
    if place.ant != None:
        if not place.ant.is_container:
            ants.append(place.ant)
        elif place.ant.is_container and place.ant.ant_contained != None:
            ants.append(place.ant)
            ants.append(place.ant.ant_contained)
        elif place.ant.is_container and place.ant.ant_contained == None:
            ants.append(place.ant)
    if place.exit != None:
        return ants + find_behind_ants(place.exit)
    return ants
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        super().action(gamestate)
        for ant in find_behind_ants(self.place.exit):
            ant.double()
        # END Problem 12
...
# Statuses #
############
def dict_of_all_insects_in_place(place, distance):
    return_dict = {}
    if len(place.bees) > 0:
        for bee in place.bees:
            return_dict.update({bee: distance})
    if place.ant != None and place.ant.is_container and place.ant.ant_contained != None:
        return_dict.update({place.ant: distance,
                            place.ant.ant_contained: distance})
    if place.ant != None:
        return_dict[place.ant] = distance
    print(f"DEBUG: 0 {return_dict}")
    return return_dict
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, place = None, distance = 0, return_dict = None):
        # BEGIN Problem Optional 2
        if distance == 0:
            return_dict = {}
            place = self.place
        return_dict.update(dict_of_all_insects_in_place(place, distance))
        if distance == 0:
            return_dict.pop(self)
        print(f"DEBUG: 1 {return_dict}")
        if place.entrance != None and not place.entrance.is_hive: #recur
            self.insects_in_front(place.entrance, distance + 1, return_dict)
        return return_dict            
        # END Problem Optional 2

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return 2 - distance*0.25 - self.insects_shot*0.0625
        # END Problem Optional 2
